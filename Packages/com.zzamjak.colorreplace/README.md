...
```
https://github.com/zzamjak-cloud/ColorReplace.git?path=/Packages/com.zzamjak.colorreplace#v1.0.0
```
...
{
  "dependencies": {
    "com.zzamjak.colorreplace": "https://github.com/zzamjak-cloud/ColorReplace.git?path=/Packages/com.zzamjak.colorreplace#v1.0.0"
  }
}
```
...
> 저장된 머티리얼 에셋은 사용하는 프로젝트의 `Assets/Plugins/CAT/ColorReplace/Materials/`에
> 자동 생성됩니다 (폴더가 없으면 에디터가 자동 생성).
>
> 이 패키지는 외부 의존성이 없습니다. SoftMaskLight / mob-sakai SoftMask의 소프트(알파) 마스킹까지
> 적용하려면 별도의 Hidden 변형 셰이더(`Hidden-CAT-ColorReplace-SoftMaskLight/SoftMaskable`)가 필요하며,
> 이는 마스크 시스템 소유 프로젝트(SoftMaskLight 플러그인)에서 관리합니다.
